...
__global__ void updateNeighbors_del(RT_Vertex* SSSP, int* updated_counter_del, int* updatedEdgesList_del, int* changedEdgesList_del, int* counter_del, int* changedEdgesList, int* counter, ColWt* AdjListFull_device, int* AdjListTracker_device, int* change) {
	int index = threadIdx.x + blockIdx.x * blockDim.x;
